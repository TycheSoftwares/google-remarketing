...
<style>
    ...
    #the-list .deactivate > .wgr-ts-slug {
        display: none
    }
    ...
    ( function ($) {
        ...
        var reasonsHtml = <?php echo json_encode( $reasons_list_items_html ); ?>,
            modalHtml =
                ...
                + '         <div class="ts-modal-panel active" data-panel-id="reasons"><h3><strong><?php printf( WP_Google_Remarketing_TS_deactivate::load_str( 'deactivation-share-reason' ) ); ?>:</strong></h3><ul id="reasons-list">' + reasonsHtml + '</ul></div>'
                + '     </div>'
                + '     <div class="ts-modal-footer">'
                + '         <a href="#" class="button button-secondary button-deactivate"></a>'
                + '         <a href="#" class="button button-primary button-close"><?php printf( WP_Google_Remarketing_TS_deactivate::load_str( 'deactivation-modal-button-cancel' ) ); ?></a>'
                + '     </div>'
                + ' </div>'
                + '</div>',
            $modal = $(modalHtml),

            $deactivateLink = $('#the-list .deactivate > .wgr-ts-slug').prev();

        for( var i = 0; i < $deactivateLink.length; i++ ) {
            $deactivateLinks[ $( $deactivateLink[i] ).siblings( ".wgr-ts-slug" ).attr( 'data-slug' ) ] = $deactivateLink[i].href;
        }
        ...
        function registerEventHandlers() {
            $deactivateLink.on( "click", function (evt) {
                evt.preventDefault();
                currentPluginName = $(this).siblings( ".wgr-ts-slug" ).attr( 'data-slug' );
                showModal();
            });
            ...
            $modal.on('click', 'input[type="radio"]', function () {
                ...
                $modal.find('.button-deactivate').text('<?php printf( WP_Google_Remarketing_TS_deactivate::load_str( 'deactivation-modal-button-submit' ) ); ?>');
        ...
        function updateButtonLabels() {
            ...
            if ('confirm' === getCurrentPanel()) {
                $deactivateButton.text('<?php printf( WP_Google_Remarketing_TS_deactivate::load_str( 'deactivation-modal-button-confirm' ) ); ?>');
            } else {
                var $radio = $('input[type="radio"]:checked');
                if( $radio.length == 0 ) {
                    $deactivateButton.text('<?php printf( WP_Google_Remarketing_TS_deactivate::load_str( 'deactivation-modal-button-deactivate' ) ); ?>');
                } else {
